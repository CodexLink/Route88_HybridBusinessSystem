...
class Route88_LoginCore(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.Login_TechnicalCoreHandler.MySQL_ConnectDatabase()
        self.Login_TechnicalCoreHandler.MySQL_CursorSet(MySQL.cursors.DictCursor)
    ...
    def LoginForm_ParseUserEnlisted(self):
        try:
            currentRow = 0
            self.Login_TechnicalCoreHandler.MySQL_CursorSet(MySQL.cursors.DictCursor)
            self.Login_TechnicalCoreHandler.MySQLDataWireCursor.execute("SELECT * FROM Employees")
            UserDataTable = self.Login_TechnicalCoreHandler.MySQLDataWireCursor.fetchall()
    ...
    def LoginForm_DataSubmission(self):
        try:
            self.Login_TechnicalCoreHandler.MySQL_CursorSet(None)
            ...
            for RowIndexQuery in sorted(RowIndexSelected):
                QueryReturn = self.Login_TechnicalCoreHandler.MySQLDataWireCursor.execute("SELECT fname, lname FROM Employees WHERE concat(lname, ', ', fname) = %s AND password = %s", (
                ...
                if QueryReturn:
                    self.Route88_LoginWindow.StatusLabel.setText("Login Success: Credential Input Matched!")
                    QSound.play("SysSounds/LoginSuccessNotify.wav")
                    QtTest.QTest.qWait(1500)
                    self.Route88_LoginWindow.StatusLabel.setText("Successfully Logged in as ...".format("Unknown User..."))
                    QtTest.QTest.qWait(1500)
                    self.Login_TechnicalCoreHandler.MySQLDataWire.close() # Reconnect to Anothe SQ: Usage with Specific User Parameters
                    self.close()
                    self.InventorySys_Init()

                else:
                    self.Route88_LoginWindow.StatusLabel.setText("Login Error: Credential Input Not Matched! Check your Password!")
...
if __name__ == "__main__":
    ...
    Route88_CoreLoad = Route88_LoginCore()
